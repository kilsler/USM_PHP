        ...
        <div class="absolute top-0 end-0 mt-3 me-3 ">
            <ul class="nav nav-tabs">
                ...
                <?php if (isset($_COOKIE['auth_token'])) : ?>
                    <li class="nav-item">
                        <a href="<?= '/logout' ?>" class=" nav-link text-black <?php if ($_SERVER['REQUEST_URI'] === '/logout') echo 'active'; ?>">Выйти из аккаунта</a>
                    </li>
                <?php else : ?>
                    <li class="nav-item">
                        <a href="<?= '/login' ?>" class=" nav-link text-black <?php if ($_SERVER['REQUEST_URI'] === '/login') echo 'active'; ?>">Войти в аккаунт</a>
                    </li>
                <?php endif; ?>
            </ul>
        </div>
...
<?php if (isset($user)): ?>
    <p>Вы вошли как <?= htmlspecialchars($user['login']) ?> | <a href="/logout.php">Выйти</a></p>
<?php endif; ?>
