...
function getRecipeByUserId()
{
    ...
    if ($conn) {
        $sql = "SELECT 
         ...
         ON recipes.category = categories.id WHERE user_id = :user_id";
...
function createRecipe($recipe)
{
    $conn = getConnection();
    if ($conn) {
        $sql = "INSERT INTO recipes (title, category, ingredients,description,tags,steps,author_id) VALUES (:title, :category, :ingredients,:description,:tags,:steps,:author_id)";
        $stmt = $conn->prepare($sql);
        ...
        $stmt->bindParam(':author_id', $_COOKIE['user_id'], PDO::PARAM_INT);
        return $stmt->execute();
    }
    return false;
}
